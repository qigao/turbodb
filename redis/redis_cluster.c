...
typedef enum redis_cluster_phase {
  ...
  REDIS_CLUSTER_PHASE_DISCOVERY_CLEANUP,
...
static uint16_t redis_cluster_crc16(const char *data, size_t length) {
  ...
  for (index = 0; index < length; ++index)
    crc = (uint16_t)((crc << 8u) ^
                     redis_cluster_crc16_table[((crc >> 8u) ^ (uint8_t)data[index]) & 0xffu]);
  return crc;
}
...
uint16_t redis_cluster_keyslot(const char *key, size_t length) {
  ...
  if (start < length) {
    ...
    if (end < length && end > start + 1u)
      return (uint16_t)(redis_cluster_crc16(key + start + 1u, end - start - 1u) & 0x3fffu);
  }
...
static const redis_cluster_impl *redis_cluster_get_const(const redis_cluster *cluster) {
  return cluster ? (const redis_cluster_impl *)cluster->impl : NULL;
}

static redis_cluster_connect_step redis_cluster_result(const redis_cluster_impl *impl,
...
static redis_pool_config redis_cluster_pool_config(const redis_cluster_impl *impl, const char *host,
                                                   uint16_t port, size_t capacity) {
...
static int redis_cluster_reply_string(const redis_reply_t *reply, const char **value) {
  if (!reply || !value ||
...
static int redis_cluster_find_or_add_node(redis_cluster_impl *impl, const char *host, uint16_t port,
  ...
  if (impl->node_count == impl->max_nodes) return SALTS_ENOBUFS;
  host_copy = redis_cluster_copy(host);
  if (!host_copy) return SALTS_ENOMEM;
  if (node_id) {
...
static int redis_cluster_parse_slots(redis_cluster_impl *impl, const redis_reply_t *reply) {
  size_t range_index;
  if (!reply || reply->type != REDIS_REPLY_ARRAY || reply->element_count == 0u) return SALTS_EPROTO;
  memset(impl->slots, 0, REDIS_CLUSTER_SLOT_COUNT * sizeof(*impl->slots));
  for (range_index = 0u; range_index < reply->element_count; ++range_index) {
    ...
    redis_cluster_node_impl *node;
    size_t slot;
    if (!range || range->type != REDIS_REPLY_ARRAY || range->element_count < 3u ||
        !range->elements[0] || !range->elements[1] ||
        range->elements[0]->type != REDIS_REPLY_INTEGER ||
        range->elements[1]->type != REDIS_REPLY_INTEGER || range->elements[0]->integer < 0 ||
    ...
    if (!endpoint || endpoint->type != REDIS_REPLY_ARRAY || endpoint->element_count < 2u ||
        redis_cluster_reply_string(endpoint->elements[0], &host) != SALTS_OK ||
        !endpoint->elements[1] || endpoint->elements[1]->type != REDIS_REPLY_INTEGER ||
        endpoint->elements[1]->integer <= 0 || endpoint->elements[1]->integer > UINT16_MAX)
      return SALTS_EPROTO;
    if (endpoint->element_count > 2u && endpoint->elements[2] &&
        endpoint->elements[2]->type != REDIS_REPLY_NULL &&
        redis_cluster_reply_string(endpoint->elements[2], &node_id) != SALTS_OK)
      return SALTS_EPROTO;
    start = (uint16_t)range->elements[0]->integer;
    end = (uint16_t)range->elements[1]->integer;
    port = (uint16_t)endpoint->elements[1]->integer;
    {
      int status = redis_cluster_find_or_add_node(impl, host, port, node_id, &node);
      if (status != SALTS_OK) return status;
    }
    if (start < node->view.slot_start) node->view.slot_start = start;
    if (end > node->view.slot_end) node->view.slot_end = end;
    for (slot = start; slot <= end; ++slot) {
      if (impl->slots[slot] && impl->slots[slot] != node) return SALTS_EPROTO;
      impl->slots[slot] = node;
    }
  }
  for (range_index = 0u; range_index < REDIS_CLUSTER_SLOT_COUNT; ++range_index) {
    if (!impl->slots[range_index]) return SALTS_EPROTO;
  }
  return SALTS_OK;
}

int redis_cluster_init(redis_cluster *cluster, const redis_cluster_config *config) {
  redis_cluster_impl *impl;
  size_t index;
  if (!cluster || cluster->impl || !config || !redis_io_runtime_valid(config->runtime) ||
      !config->seed_hosts || !config->seed_ports || config->seed_count == 0u ||
      ...
      config->receive_chunk_bytes == 0u || config->topology_reply_bytes == 0u ||
      config->cancel_timeout_ns == 0u ||
      config->max_nodes > (SIZE_MAX - 1u) / config->connections_per_node)
    return SALTS_EINVAL;
  if (config->max_nodes > SIZE_MAX / sizeof(redis_cluster_node_impl) ||
  ...
  impl->seed_ports = (uint16_t *)calloc(config->seed_count, sizeof(uint16_t));
  impl->nodes = (redis_cluster_node_impl *)calloc(config->max_nodes, sizeof(*impl->nodes));
  impl->slots = (redis_cluster_node_impl **)calloc(REDIS_CLUSTER_SLOT_COUNT, sizeof(*impl->slots));
  ...
  {
    redis_pool_config seed_config =
        redis_cluster_pool_config(impl, impl->seed_hosts[0], impl->seed_ports[0], 1u);
    if (redis_pool_init(&impl->seed_pool, &seed_config) != SALTS_OK) goto invalid;
  }
  cluster->impl = impl;
  return SALTS_OK;
...
redis_cluster_connect_step redis_cluster_connect_next(redis_cluster *cluster) {
  ...
  if (!impl)
    return redis_cluster_result(NULL, REDIS_CLUSTER_CONNECT_ERROR, SALTS_EINVAL, empty_waitable);
  if (impl->phase == REDIS_CLUSTER_PHASE_READY)
    return redis_cluster_result(impl, REDIS_CLUSTER_CONNECT_DONE, SALTS_OK, empty_waitable);
  if (impl->phase == REDIS_CLUSTER_PHASE_SEED_CONNECT) {
    ...
    if (seed.kind == REDIS_POOL_CONNECT_WAIT)
      return redis_cluster_result(impl, REDIS_CLUSTER_CONNECT_WAIT, SALTS_OK, seed.waitable);
    if (seed.kind == REDIS_POOL_CONNECT_ERROR) {
      if (seed.status != SALTS_EBUSY) impl->phase = REDIS_CLUSTER_PHASE_FAILED;
      return redis_cluster_result(impl, REDIS_CLUSTER_CONNECT_ERROR, seed.status, empty_waitable);
    }
    {
      int status = redis_pool_command_open(&impl->seed_pool, 2, topology_command, NULL,
                                           impl->topology_reply_bytes, &impl->discovery);
      if (status != SALTS_OK) {
        impl->phase = REDIS_CLUSTER_PHASE_FAILED;
        return redis_cluster_result(impl, REDIS_CLUSTER_CONNECT_ERROR, status, empty_waitable);
      }
    }
  ...
  if (impl->phase == REDIS_CLUSTER_PHASE_DISCOVERY) {
    for (;;) {
      redis_cflow_stream_step discovered = redis_pool_stream_next(&impl->discovery);
      if (discovered.kind == REDIS_CFLOW_STREAM_WAIT)
        return redis_cluster_result(impl, REDIS_CLUSTER_CONNECT_WAIT, SALTS_OK,
                                    discovered.waitable);
      ...
      impl->phase = REDIS_CLUSTER_PHASE_DISCOVERY_CLEANUP;
      break;
    }
  }
  if (impl->phase == REDIS_CLUSTER_PHASE_DISCOVERY_CLEANUP) {
    if (impl->discovery.impl) {
      cleanup_status = redis_pool_stream_destroy(&impl->discovery);
      if (cleanup_status != SALTS_OK) goto cleanup_error;
    }
    if (impl->seed_pool.impl) {
      cleanup_status = redis_pool_close(&impl->seed_pool);
      if (cleanup_status != SALTS_OK) goto cleanup_error;
      cleanup_status = redis_pool_destroy(&impl->seed_pool);
      if (cleanup_status != SALTS_OK) goto cleanup_error;
    }
    impl->phase = REDIS_CLUSTER_PHASE_NODE_CONNECT;
  }
  if (impl->phase != REDIS_CLUSTER_PHASE_NODE_CONNECT)
    return redis_cluster_result(impl, REDIS_CLUSTER_CONNECT_ERROR, SALTS_ESHUTDOWN, empty_waitable);
  while (impl->phase == REDIS_CLUSTER_PHASE_NODE_CONNECT && impl->connect_node < impl->node_count) {
    redis_cluster_node_impl *node = &impl->nodes[impl->connect_node];
    if (!node->pool.impl) {
      redis_pool_config config =
          redis_cluster_pool_config(impl, node->host, node->view.port, impl->connections_per_node);
    ...
    }
    {
      ...
      if (connected.kind == REDIS_POOL_CONNECT_WAIT)
        return redis_cluster_result(impl, REDIS_CLUSTER_CONNECT_WAIT, SALTS_OK, connected.waitable);
      if (connected.kind == REDIS_POOL_CONNECT_ERROR) {
        if (connected.status != SALTS_EBUSY) impl->phase = REDIS_CLUSTER_PHASE_FAILED;
        return redis_cluster_result(impl, REDIS_CLUSTER_CONNECT_ERROR, connected.status,
                                    empty_waitable);
      }
    }
    impl->connect_node++;
...
failed_protocol:
  impl->phase = REDIS_CLUSTER_PHASE_FAILED;
  return redis_cluster_result(impl, REDIS_CLUSTER_CONNECT_ERROR, SALTS_EPROTO, empty_waitable);
cleanup_error:
  if (cleanup_status != SALTS_EBUSY) impl->phase = REDIS_CLUSTER_PHASE_FAILED;
  return redis_cluster_result(impl, REDIS_CLUSTER_CONNECT_ERROR, cleanup_status, empty_waitable);
}

int redis_cluster_command_open(redis_cluster *cluster, const char *key, size_t key_length, int argc,
  ...
  if (!key || key_length == 0u) return SALTS_EINVAL;
  slot = redis_cluster_keyslot(key, key_length);
  if (!impl->slots[slot]) return SALTS_ENOTCONN;
  return redis_pool_command_open(&impl->slots[slot]->pool, argc, argv, argvlen, max_reply_bytes,
                                 out_stream);
}

const redis_cluster_node *redis_cluster_node_for_slot(const redis_cluster *cluster, uint16_t slot) {
  const redis_cluster_impl *impl = redis_cluster_get_const(cluster);
  if (!impl || slot >= REDIS_CLUSTER_SLOT_COUNT || !impl->slots[slot]) return NULL;
...
int redis_cluster_close(redis_cluster *cluster) {
  ...
  if (impl->phase == REDIS_CLUSTER_PHASE_CLOSED) return SALTS_OK;
  ...
  if (result == SALTS_OK) impl->phase = REDIS_CLUSTER_PHASE_CLOSED;
  return result;
}
