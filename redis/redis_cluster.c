...
static int redis_cluster_parse_slots(redis_cluster_impl *impl, const redis_reply_t *reply) {
  size_t range_index;
  if (!reply || reply->type != REDIS_REPLY_ARRAY || reply->element_count == 0u) return TURBO_EPROTO;
  ...
  for (range_index = 0u; range_index < reply->element_count; ++range_index) {
    ...
    for (slot = start; slot <= end; ++slot) {
      if (impl->slots[slot] && impl->slots[slot] != node) return TURBO_EPROTO;
...
int redis_cluster_init(redis_cluster *cluster, const redis_cluster_config *config) {
  ...
  impl->phase = REDIS_CLUSTER_PHASE_SEED_CONNECT;
  {
    ...
    if (redis_pool_init(&impl->seed_pool, &seed_config) != TURBO_OK) goto invalid;
  }
  cluster->impl = impl;
  return TURBO_OK;
...
redis_cluster_connect_step redis_cluster_connect_next(redis_cluster *cluster) {
  ...
  if (!impl)
    return redis_cluster_result(NULL, REDIS_CLUSTER_CONNECT_ERROR, TURBO_EINVAL, empty_waitable);
  if (impl->phase == REDIS_CLUSTER_PHASE_READY)
    return redis_cluster_result(impl, REDIS_CLUSTER_CONNECT_DONE, TURBO_OK, empty_waitable);
  if (impl->phase == REDIS_CLUSTER_PHASE_SEED_CONNECT) {
    ...
    if (seed.kind == REDIS_POOL_CONNECT_WAIT)
      return redis_cluster_result(impl, REDIS_CLUSTER_CONNECT_WAIT, TURBO_OK, seed.waitable);
    if (seed.kind == REDIS_POOL_CONNECT_ERROR) {
      if (seed.status != TURBO_EBUSY) impl->phase = REDIS_CLUSTER_PHASE_FAILED;
  ...
  if (impl->phase != REDIS_CLUSTER_PHASE_NODE_CONNECT)
    return redis_cluster_result(impl, REDIS_CLUSTER_CONNECT_ERROR, TURBO_ESHUTDOWN, empty_waitable);
  while (impl->phase == REDIS_CLUSTER_PHASE_NODE_CONNECT && impl->connect_node < impl->node_count) {
    ...
    }
    {
      ...
      if (connected.kind == REDIS_POOL_CONNECT_WAIT)
        return redis_cluster_result(impl, REDIS_CLUSTER_CONNECT_WAIT, TURBO_OK, connected.waitable);
      if (connected.kind == REDIS_POOL_CONNECT_ERROR) {
        if (connected.status != TURBO_EBUSY) impl->phase = REDIS_CLUSTER_PHASE_FAILED;
...
failed_protocol:
  impl->phase = REDIS_CLUSTER_PHASE_FAILED;
  return redis_cluster_result(impl, REDIS_CLUSTER_CONNECT_ERROR, TURBO_EPROTO, empty_waitable);
cleanup_error:
  if (cleanup_status != TURBO_EBUSY) impl->phase = REDIS_CLUSTER_PHASE_FAILED;
