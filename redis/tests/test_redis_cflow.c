...
static int redis_cflow_test_nonblocking(redis_cflow_test_socket socket_value) {
  ...
  return ioctlsocket(socket_value, FIONBIO, &enabled) == 0
             ? TURBO_OK : redis_cflow_test_socket_error();
#else
...
static int redis_cflow_test_pair(redis_cflow_test_socket sockets[2]) {
  ...
  if (status == TURBO_OK) {
    ...
    if (sockets[0] == REDIS_CFLOW_TEST_INVALID)
      status = redis_cflow_test_socket_error();
  }
  if (status == TURBO_OK &&
  ...
  if (status == TURBO_OK) {
    ...
    if (sockets[1] == REDIS_CFLOW_TEST_INVALID)
      status = redis_cflow_test_socket_error();
  }
...
static int redis_cflow_test_listener(redis_cflow_test_socket *listener,
                                     uint16_t *port) {
  struct sockaddr_in address;
#if defined(_WIN32)
  int address_size = (int)sizeof(address);
...
#endif
  *listener = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
  if (*listener == REDIS_CFLOW_TEST_INVALID)
    return redis_cflow_test_socket_error();
  memset(&address, 0, sizeof(address));
  address.sin_family = AF_INET;
  address.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
  if (bind(*listener, (const struct sockaddr *)&address,
...
suite("redis CFlow RESP stream") {
  it("connects a resolved endpoint through the CFlow native backend") {
    ...
    config = (redis_cflow_open_config){
        &runtime, "127.0.0.1", port, 7u, 2u, 1024u, 8u, 128u, 16u,
        UINT64_C(5000000000)};
  ...
  it("turns partial network RESP into demand-driven WAIT and ITEM steps") {
    ...
    connection_config = (redis_cflow_connection_config){
        &runtime, (uintptr_t)sockets[0], 1u, 1024u, 8u, 128u, 16u,
        UINT64_C(5000000000), 1};
    check_equal(redis_cflow_connection_init_attached(&connection,
                                                     &connection_config),
                TURBO_OK);
    check_equal(redis_cflow_stream_open(&connection, 1, arguments, NULL,
                                        1024u, 2u, &stream),
                TURBO_OK);
    ...
    check_equal(send(sockets[1], first_reply,
                     (int)(sizeof(first_reply) - 1u), 0),
    ...
    step = redis_cflow_stream_next(&stream);
    check_equal(step.kind, REDIS_CFLOW_STREAM_WAIT);
    check_equal(send(sockets[1], second_reply,
                     (int)(sizeof(second_reply) - 1u), 0),
    ...
    check_equal(redis_cflow_command_open(&connection, 1, arguments, NULL,
                                         1024u, &stream),
                TURBO_OK);
    step = redis_cflow_stream_next(&stream);
    check_equal(step.kind, REDIS_CFLOW_STREAM_WAIT);
  ...
  it("invalidates a connection when an in-flight partial reply is cancelled") {
    ...
    check_equal(redis_cflow_test_pair(sockets), TURBO_OK);
    connection_config = (redis_cflow_connection_config){
        &runtime, (uintptr_t)sockets[0], 2u, 1024u, 8u, 128u, 16u,
        UINT64_C(5000000000), 1};
    check_equal(redis_cflow_connection_init_attached(&connection,
                                                     &connection_config),
                TURBO_OK);
    ...
    check_equal(send(sockets[1], partial_reply,
                     (int)(sizeof(partial_reply) - 1u), 0),
  ...
  it("transfers a server error reply exactly once") {
    ...
    check_equal(redis_cflow_test_pair(sockets), TURBO_OK);
    connection_config = (redis_cflow_connection_config){
        &runtime, (uintptr_t)sockets[0], 4u, 1024u, 8u, 128u, 16u,
        UINT64_C(5000000000), 1};
    check_equal(redis_cflow_connection_init_attached(&connection,
                                                     &connection_config),
                TURBO_OK);
    check_equal(redis_cflow_command_open(&connection, 2, arguments, NULL,
                                         1024u, &stream),
                TURBO_OK);
    step = redis_cflow_stream_next(&stream);
    check_equal(step.kind, REDIS_CFLOW_STREAM_WAIT);
    ...
    check_equal(step.kind, REDIS_CFLOW_STREAM_WAIT);
    check_true(recv(sockets[1], command, (int)sizeof(command), 0) > 0);
    check_equal(send(sockets[1], server_error,
                     (int)(sizeof(server_error) - 1u), 0),
  ...
  it("reports I/O admission saturation separately from a busy lease") {
    static const char *arguments[] = {"PING"};
    ...
    redis_cflow_stream stream = {0};
    redis_cflow_stream_step step;
    redis_io_request occupied = {0};
    cflow_io_completion completion;
    cflow_io_native_operation operation = {
        .kind = CFLOW_IO_NATIVE_TCP_RECV,
        .socket = (uintptr_t)REDIS_CFLOW_TEST_INVALID,
        .buffer = &pending_byte,
        .length = 1u};

    check_equal(redis_io_runtime_init(&runtime, &runtime_config), TURBO_OK);
    check_equal(redis_cflow_test_pair(sockets), TURBO_OK);
    operation.socket = (uintptr_t)sockets[1];
    check_equal(redis_io_runtime_try_submit(&runtime, 40u, &operation,
                                            &occupied),
                REDIS_IO_SUBMIT_ACCEPTED);
    connection_config = (redis_cflow_connection_config){
        &runtime, (uintptr_t)sockets[0], 41u, 1024u, 8u, 128u, 16u,
        UINT64_C(5000000000), 1};
    check_equal(redis_cflow_connection_init_attached(&connection,
                                                     &connection_config),
                TURBO_OK);
    ...
    step = redis_cflow_stream_next(&stream);
    check_equal(step.kind, REDIS_CFLOW_STREAM_ERROR);
    check_equal(step.status, TURBO_ENOBUFS);
    check_equal(redis_cflow_stream_destroy(&stream), TURBO_OK);
    check_equal(redis_cflow_connection_destroy(&connection), TURBO_OK);
    check_equal(redis_io_request_cancel(&occupied),
                CFLOW_IO_CANCEL_ACCEPTED);
    check_equal(redis_io_runtime_wait_idle(&runtime, UINT64_C(5000000000)),
                TURBO_OK);
    check_equal(redis_io_request_poll(&occupied, &completion),
                REDIS_IO_REQUEST_COMPLETED);
    check_equal(redis_io_request_acknowledge(&occupied), TURBO_OK);
    redis_cflow_test_close(sockets[1]);
    check_equal(redis_io_runtime_close(&runtime), TURBO_OK);
    check_equal(redis_io_runtime_destroy(&runtime), TURBO_OK);
  }
