...
suite("redis CFlow connection pool") {
  it("rejects invalid fixed-capacity configuration") {
    ...
    check_equal(redis_pool_init(&pool, &config), TURBO_EINVAL);
  ...
  it("holds one bounded lease until the command stream reaches terminal") {
    ...
    check_equal(redis_pool_stream_destroy(&first), TURBO_OK);
    ...
    step = redis_pool_test_next(&second, &runtime);
    check_equal(step.kind, REDIS_CFLOW_STREAM_DONE);
    check_equal(redis_pool_stream_destroy(&second), TURBO_OK);
    ...
    check_equal(redis_pool_close(&pool), TURBO_OK);
    check_equal(redis_pool_destroy(&pool), TURBO_OK);
  ...
  it("reconnects an invalid slot through the pool connect state machine") {
    ...
    step = redis_pool_stream_next(&cancelled);
    check_equal(step.kind, REDIS_CFLOW_STREAM_WAIT);
    check_equal(redis_pool_stream_cancel(&cancelled), TURBO_OK);
    check_equal(redis_pool_stream_destroy(&cancelled), TURBO_OK);
    ...
    check_equal(connected.status, TURBO_OK);
    {
      ...
      if (opened == TURBO_OK) {
        ...
        step = redis_pool_test_next(&recovered, &runtime);
        check_equal(step.kind, REDIS_CFLOW_STREAM_DONE);
        check_equal(redis_pool_stream_destroy(&recovered), TURBO_OK);
      }
    }
