...
static const char *orm_command_source_name(void *state) {
  (void)state;
  return "orm-command";
}

static const cmeta_type_desc *orm_command_source_output_type(void *state) {
  (void)state;
  return &orm_command_result_type;
}

static cflow_step orm_command_source_resume(void *state_,
                                            cflow_resume_ctx *context,
                                            void *out_value) {
  orm_command_source_state *state = (orm_command_source_state *)state_;
...
static void orm_command_source_cancel(void *state_) {
  orm_command_source_state *state = (orm_command_source_state *)state_;
  if (state->terminal == CFLOW_SOURCE_OPEN)
    state->terminal = CFLOW_SOURCE_DONE;
}

static void orm_command_source_destroy(void *state_) {
  orm_command_source_state *state = (orm_command_source_state *)state_;
  if (state == NULL)
    return;
...
static void orm_command_source_bind_terminal_waker(void *state,
                                                   cflow_waker waker) {
  (void)state;
  (void)waker;
}

static cflow_source_terminal orm_command_source_poll_terminal(
    void *state_, const char **error) {
  const orm_command_source_state *state =
      (const orm_command_source_state *)state_;
  if (error != NULL)
    *error = state->terminal == CFLOW_SOURCE_ERROR
                 ? state->error_message
                 : NULL;
  return state->terminal;
}

CMETA_IMPLEMENTS(cflow_source, orm_command_source,
                 CFLOW_SOURCE_CAP_CONSTRUCTS_VALUES,
    .name = orm_command_source_name,
    .output_type = orm_command_source_output_type,
    .resume = orm_command_source_resume,
    .cancel = orm_command_source_cancel,
    .destroy = orm_command_source_destroy,
    .bind_terminal_waker = orm_command_source_bind_terminal_waker,
    .poll_terminal = orm_command_source_poll_terminal
);

orm_status_t orm_command_source_init(cflow_source *out_source,
                                     orm_command_driver *driver,
                                     orm_error_t *error) {
  orm_command_source_state *state;
  if (out_source == NULL || cflow_source_valid(out_source) ||
      !orm_command_driver_valid(driver)) {
    orm_command_set_error(error, ORM_STATUS_INVALID_ARGUMENT,
                          "invalid command Source configuration");
    return ORM_STATUS_INVALID_ARGUMENT;
  }
  state = (orm_command_source_state *)calloc(1u, sizeof(*state));
  if (state == NULL) {
  ...
  *out_source = orm_command_source_as_cflow_source(state);
  driver->ops = NULL;
  driver->context = NULL;
